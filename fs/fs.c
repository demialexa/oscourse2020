...
block_is_free(uint32_t blockno) {
    ...
    if (bitmap[blockno / 32] & (1U << (blockno % 32))) return 1;
    return 0;
}
...
file_truncate_blocks(struct File *f, off_t newsize) {
    uint32_t old_nblocks = (f->f_size + BLKSIZE - 1) / BLKSIZE;
    uint32_t new_nblocks = (newsize + BLKSIZE - 1) / BLKSIZE;
    for (uint32_t bno = new_nblocks; bno < old_nblocks; bno++) {
