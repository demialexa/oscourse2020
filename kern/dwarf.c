      ...
      if (!strcmp(fname, (const char *)pubnames_entry)) {
        ...
        if (tag == DW_TAG_subprogram) {
          ...
          do {
            abbrev_entry += dwarf_read_uleb128(abbrev_entry, &name);
            abbrev_entry += dwarf_read_uleb128(abbrev_entry, &form);
            entry += dwarf_read_abbrev_entry(entry, form,
                name == DW_AT_low_pc ? &low_pc : NULL,
                name == DW_AT_low_pc ? sizeof(low_pc) : 0, address_size);
          } while (name || form);
          *offset = low_pc;
        } else {
    ...
    while (entry < entry_end) {
      ...
      if (tag == DW_TAG_subprogram || tag == DW_TAG_label) {
        ...
        do {
          ...
          } else entry += dwarf_read_abbrev_entry(entry, form, NULL, 0, address_size);
        } while (name || form);
        if (found) {
