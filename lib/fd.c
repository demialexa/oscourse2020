...
dup(int oldfdnum, int newfdnum) {
    ...
    pte_t oldvaent = get_uvpt_entry(oldva);
    if (oldvaent & PTE_P) {
        if ((res = sys_page_map(0, oldva, 0, newva, oldvaent & PTE_SYSCALL)) < 0) goto err;
    }
    pte_t oldfdent = get_uvpt_entry(oldfd);
    if ((res = sys_page_map(0, oldfd, 0, newfd, oldfdent & PTE_SYSCALL)) < 0) goto err;

    return newfdnum;

err:
    sys_page_unmap(0, newfd);
    sys_page_unmap(0, newva);
    return res;
}
