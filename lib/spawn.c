...
spawn(const char *prog, const char **argv) {
    ...
    if (res != sizeof(elf_buf)) {
        cprintf("Wrong ELF header size or read error: %d", res);
        close(fd);
        return -E_NOT_EXEC;
    }
    ...
    if (res < 0) goto error;
    res = map_segment(child, SANITIZE_USER_STACK_SHADOW_BASE, SANITIZE_USER_STACK_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
    if (res < 0) goto error;
    res = map_segment(child, SANITIZE_USER_EXTRA_SHADOW_BASE, SANITIZE_USER_EXTRA_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
    if (res < 0) goto error;
    res = map_segment(child, SANITIZE_USER_FS_SHADOW_BASE, SANITIZE_USER_FS_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
    if (res < 0) goto error;
    ...
    if ((res = copy_shared_pages(child)) < 0)
        panic("copy_shared_pages: %i", res);
