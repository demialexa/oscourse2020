...
spawn(const char *prog, const char **argv) {
  ...
  if ((r = init_stack(child, argv, prsp)) < 0) return r;
  ...
  r = map_segment(child, SANITIZE_USER_SHADOW_BASE, SANITIZE_USER_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
  if (r < 0) goto error;
  r = map_segment(child, SANITIZE_USER_STACK_SHADOW_BASE, SANITIZE_USER_STACK_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
  if (r < 0) goto error;
  r = map_segment(child, SANITIZE_USER_EXTRA_SHADOW_BASE, SANITIZE_USER_EXTRA_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
  if (r < 0) goto error;
  r = map_segment(child, SANITIZE_USER_FS_SHADOW_BASE, SANITIZE_USER_FS_SHADOW_SIZE, fd, 0, 0, PTE_P | PTE_U | PTE_W);
  if (r < 0) goto error;
  ...
  *init_esp = UTEMP2USTACK(&argv_store[-2]);
  ...
  for (i = 0; i < USTACKSIZE; i += PGSIZE) {
    if ((r = sys_page_map(0, UTEMP + i, child,
         (void *)(USTACKTOP - USTACKSIZE + i), PTE_P | PTE_U | PTE_W)) < 0) goto error;
    if ((r = sys_page_unmap(0, UTEMP + i)) < 0) goto error;
  }
